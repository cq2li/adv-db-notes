...
\begin{itemize}
    ...
    process. \\
    Example: \dbSys{VectorWise}~\cite{bonc05}, \dbSys{Peloton}~\cite{menon17}.
\end{itemize}
...
\begin{itemize}
    ...
    \item
    The evaluation of the \dbSys{HIQUE}~\cite{krikellas10} system shows that the DBMS incurs fewer 
...
One solution to mask the compilation time is \dbSys{HyPer}'s \textit{Adaptive Execution} 
model~\cite{kohn18}:
\begin{enumerate}
    \item
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Query accelerator for \dbSys{Postgres}/\dbSys{Greenplum} that uses LLVM + intra-query 
parallelism
        \item JIT Predicates
    \end{itemize}
    \item Apache Spark
